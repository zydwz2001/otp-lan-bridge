...
> 手机收到短信验证码后，传到电脑浏览器填写。支持同一 Wi‑Fi，也支持授权后的 USB 自动连接。
...
| 项目 | 验证码传递（Android / Chrome 2.3.3） |
| --- | --- |
| 连接方式 | 自动优先使用已授权 USB；不可用时尝试 Wi‑Fi |
| VPN / Tailscale | 不需要，也不会创建或启动 VPN |
| USB / ADB | 浏览器 WebUSB + ADB，无需电脑辅助程序；首次需 USB 调试及设备授权 |
| Android 应用 ID | `io.github.zydwz2001.wifiotprelay` |
| App 监听端口 | App 当前显示的端口（当前版本通常为 `42871`） |
| 产品界面 | 自动选择连接方式；App 底部及插件设置内提供简洁的数据线使用指引 |
...
- 使用 Wi‑Fi 时，手机与电脑必须连接同一个可互访的网络。宾馆、学校、公司访客网如果开启了“客户端隔离”，即使 Wi‑Fi 名称相同也可能无法直连。
- 使用 USB 时，建议 App 和插件均更新至 2.3.3；需要可传数据的线、Android USB 调试、手机授权电脑，以及 Chrome 首次授权设备。无需两端连接同一 Wi‑Fi。

## 没有 Wi-Fi？用数据线连接

1. **手机开启 USB 调试**：打开 App，滑到最下方“无 Wi-Fi 使用方法”，点击“开启 USB 调试”，按提示打开系统开关。返回 App 后，已开启会显示绿色 √。
2. **插上数据线**：连接手机和电脑，手机弹出“允许 USB 调试”时选择允许。
3. **浏览器选择手机**：在插件设置中点击“配对手机”（已配对则点“重新连接”），再点击连接窗口里的“连接手机”，在设备列表中选择手机并允许连接。

USB 调试通常位于：**设置 → 系统 / 更多设置 → 开发者选项 → USB 调试**。如果没有“开发者选项”，先进入“关于手机”，连续点击“版本号”7 次；小米可在“全部参数与信息”中点击“OS 版本”或“MIUI 版本”。App 底部按钮会引导跳转。

首次仍需在 App 点“配对电脑”，在插件填写 6 位配对码；手机地址和端口可留空。已有配对无需重配。完成通知读取设置后，在 App 点“开始传递”，后续用法相同。

插件设置底部的“无 Wi-Fi？用数据线，需 USB 调试”可点击展开步骤。更多说明见 [数据线连接使用方法](docs/USB_CONNECTION.md)。
...
以下为原有 Wi‑Fi 步骤。使用数据线时仍点击相同的“配对手机”“连接手机”按钮；在 Chrome 设备选择窗口中选择手机。USB 可用后，地址和端口可以留空或保留旧值，6 位配对码仍须正确。
...
配对完成后，两端会各自保存一份配对材料。正常重启、关闭浏览器或切换网页都不需要重新配对。Chrome 冷启动时会立即连接，并通过持久化闹钟和页面活动事件兜底恢复失败的连接，不需要通过开关 VPN 触发。手机 Wi-Fi 地址发生变化时，插件会先通过已配对设备的稳定局域网名称寻找手机，同时搜索原 Wi-Fi 网段；找到后仍会用原配对密钥验证手机身份，再从加密状态消息中保存手机当前的真实 Wi-Fi 地址并重新连接。因此，更换路由器或切换到不同私有网段后也不需要重新配对。
...
3. 自己点击网站的“发送验证码”。
4. 短信到达后，手机只解析所选短信 App 的通知，并通过当前连接的加密会话发给等待标签页。等待会话建立时会回查刚到的通知，避免短信与浏览器等待指令几乎同时到达时漏码。
...
等待期间如果收到新的验证码，插件会自动替换为最新一条。验证码未被浏览器确认时，手机会自动重发；“写入验证码并登录”也会自动查找当前页面及内嵌页面中的验证码输入框，不要求提前点击。

## 权限与安全边界

- Android 不申请 `READ_SMS` 或 `RECEIVE_SMS`，只使用系统“通知使用权”读取所选短信 App 的新通知。
- Wi‑Fi 服务只绑定 Android 的 Wi‑Fi IPv4 地址，并拒绝回环、链路本地、Tailscale CGNAT 和公网来源。USB 使用单独的手机本地 `127.0.0.1:42872` 服务，同样要求配对及加密认证。
...
- 路由器若开启 AP/客户端隔离，可换用可互访的 Wi‑Fi，或使用授权后的 USB 通信。USB 不依赖校园网内部互访，也不会改变校园网策略。
...
- 如果只有下拉通知栏后才收到验证码，请更新 Android App 至 2.2.6。等待期间会每秒回查所选短信 App 的近期通知，并临时保持 CPU 唤醒；取消、断线或等待到期后停止。无需更换 Chrome 插件或重新配对。
- 通常应先点网页面板“一键填充并等待”，然后再让网站发送短信；如果两者几乎同时发生，App 会在等待建立后自动回查刚到的通知。
