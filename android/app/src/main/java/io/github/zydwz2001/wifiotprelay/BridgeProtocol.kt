...
class BridgeProtocol(
    ...
    private val config: BridgePairingStore,
    ...
    fun onOpen(connection: BridgeConnection) {
        contexts[connection] = ClientContext()
        onStateChanged("浏览器正在认证")
    }

    fun onClose(connection: BridgeConnection) {
    ...
    fun onMessage(connection: BridgeConnection, message: String) = synchronized(stateLock) {
    ...
    fun stopSafely() {
        ...
        contexts.keys.toList().forEach { it.close(1001, "Service stopped") }
        contexts.clear()
    }
    ...
    private fun handlePlain(connection: BridgeConnection, context: ClientContext, json: JSONObject) {
    ...
    private fun handlePair(connection: BridgeConnection, json: JSONObject) {
        ...
        // The client closes after receiving PAIR_OK. Closing a queued byte-stream
        // transport here could discard the response before it reaches the wire.
    }

    private fun handleAuthInit(connection: BridgeConnection, context: ClientContext, json: JSONObject) {
    ...
    private fun handleEncrypted(connection: BridgeConnection, context: ClientContext, envelope: JSONObject) {
        ...
        if (context.stage != Stage.AUTHENTICATED || synchronized(stateLock) { activeClient != connection }) {
            connection.close(1008, "Authentication required")
            return
        }
    ...
    private fun handleArm(connection: BridgeConnection, payload: JSONObject) {
        val now = System.currentTimeMillis()
    ...
    private fun handleCancel(connection: BridgeConnection, payload: JSONObject) {
        val requestId = payload.optString("requestId")
        synchronized(stateLock) {
    ...
    private fun sendStatus(connection: BridgeConnection) {
        sendEncrypted(
            connection,
            "ACK",
            JSONObject()
                ...
                .put("hostAddress", hostAddressProvider())
                .put("port", config.port)
                .put("serverTime", System.currentTimeMillis())
        )
    }

    private fun sendEncrypted(connection: BridgeConnection, type: String, payload: JSONObject): Boolean {
    ...
    private fun retryPendingOtp(preferredConnection: BridgeConnection? = null, force: Boolean = false) {
        val now = System.currentTimeMillis()
    ...
    private fun sendProtocolError(connection: BridgeConnection, context: ClientContext, code: String, message: String) {
    ...
    private fun sendPlainError(connection: BridgeConnection, code: String, message: String) {
